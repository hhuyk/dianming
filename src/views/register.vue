  ...
  <div id="register">
    <form method="POST" id="form" @submit.prevent=" send_register ">
      ...
      <div class="form-group">
        ...
        <input type="password" pattern="^(\w){6,20}$" required v-model.trim=" user.password_repeat ">
      </div>
      <div class="form-group">
        <div class="group-title">
          ...
          reg(this.user)
            .then((res)=>{
              if(res.data==='fail'){
                ...
                setCookie({ name: 'token', val: res.data.user_email });
                this.$store.commit('SET_CURUSER', res.data);
                this.$router.push('/')
              }
            })
            .catch((error) => {
